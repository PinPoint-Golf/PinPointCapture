...
| **OPEN-6** | Does v1 ship tethered-only, deferring the standalone UI? | Defensible. But the **formats** must be standalone-ready from day one — the UI can wait, the schema cannot. **Decided 24 Sep 2026: yes — v1 ships tethered-only.** PPC is a camera on a tripod that PinPoint Studio drives over PPCP; its UI shows connection and recording status and nothing else, the standalone UI is retained out of the build in `Mothballed/`, and nothing is kept on the phone past its delivery. The formats stay standalone-ready, as this row always required: the hostless paths are still built and tested. *Superseded position, 23 Aug: recommend closing as "no".* The premise has been overtaken. The standalone path is the *more* complete half of what has been built — hostless sessions, bundle write and read, and the whole zero-host path pass end to end — while the live host link is the part with no caller. Deferring the standalone UI now would defer the half that works in order to ship the half that does not. |
