...
✅ **The camera half ran on 2 September 2026** and every number that paragraph asked for is in. On an iPhone 16 at 1080p240 wide, over 15.0 s: **3594 frames, 239.5 fps against a claimed 240, max inter-arrival 4.18 ms against a 4.17 ms frame period**, zero encoder-busy drops, zero not-retaining drops, zero write failures, zero non-monotonic, 30 fragments written against 10 evicted, **20/20 held**, focus/exposure/white balance all `locked → locked` *after* the run, thermal `fair`.

⛔ **And the tier question is answered: High.** `RingStats.encodedProfileLevel` parses the `hvcC` box out of the initialisation segment — the encoder's own declaration rather than what was asked for — and reads **HEVC Main profile, High tier, level 5.1** (`profile_idc 1, tier_flag 1, level_idc 153`). Main tier caps at 40 Mbit/s at that level and this application asks for a provisional 50, so Main would have meant a stream exceeding a level a decoder is entitled to believe. ⚠ **This does not harden the bitrate** — E-M2 still owns it, and E1.4 is untouched.

⚠ **The 25 August `↕100.2 ms` did not reappear**, which retires it as the startup transient it was diagnosed to be: the largest single gap in the whole run is one frame period.

| Level | Capability | Components | Release | Exit criterion |
|---|---|---|---|---|
| **E1.1** ✅ | **Frames reach the ring** | Live `AVCaptureVideoDataOutput` → `RingBufferRecorder`, through one state-gated delegate ✅ · encoder config: `ExpectedFrameRate`, `RealTime`, `AllowFrameReordering = false` ✅ *(was already set — the earlier ○ was wrong)* · fragment index into `Core.FragmentRing`, rollover at 20 ✅ · `RingStats` — inter-arrival, evictions, uncounted frames, **and the encoded profile/level off the `hvcC` box** ✅ · verify REQ-OPT-1..7 locks hold on hardware ✅ | v1 | **Done 2 Sep 2026** (#17). Twenty 0.5 s fragments on disk, rolling, at the claimed rate with `alwaysDiscardsLateVideoFrames = false` — 20/20 held, 239.5 fps against 240, ↕4.18 ms against a 4.17 ms period, locks re-read after the run, **High tier confirmed**. See the paragraph above for the full readout |
| **E1.2** ✅ | **A clip you can extract** | Trigger → `CaptureAssembly` concatenation ✅ · `CaptureDevice.extractClip` over real fragments ✅ · `retainedClip(aroundNs:preNs:postNs:)` answering a host's `capture_request` ✅ · thumbnail at the impact anchor, zero generator tolerance ✅ | v1 | **Done 2 Sep 2026** (#18). On an iPhone 16: 15.0 MB opening as **one video track of 2.500 s**, `hvc1`, 599 frames at 239.49 fps, a 3.3 KB JPEG at the anchor. ⚠ And from the other direction — a live PinPointStudio's `capture_request` answered with 15–25 MB clips and 81 MB bundles written beside them. ⚠ Every such answer was `partial`, never `complete`; that is E3.4's question |
| **E1.3** ✅ | **A clip that is self-describing** | Full REQ-CLIP-1 sidecar: per-frame timestamps, intrinsics, attitude and gravity, exposure and ISO, thermal timeline, achieved frames, stream coverage, gaps ✅ — **carried on hardware 2 Sep** with exposure measured at 4.038 ms rather than the hardcoded zero | v1 | **Done 2 Sep 2026** (#19). ⚠ **Criterion narrowed on closing, deliberately**: it read *"on-device `make conform` unblocks CT-S7 (4), CT-S1 (1–5), CT-I30's third assertion, IOP-2's second half"*, and that run has still not happened. Mark's call, 2 September: this level's job is that a clip describes itself, and it does — driving `ppcp-conform` on the device is a **conformance-claim** obligation, not a capability E1.3 owes. It survives as E3.4's exit criterion and as `ppcp-conformance.md`'s `blocked: a phone` rows. ✅ The intrinsics overclaim is separately settled: delivery measured as available at 1080p30/60/120 and **not at 240**, with stabilisation ruled out, and the declaration now claims none at enumeration |
...
| **E3.2** ✅ | **Synchronised** | Sync burst 10–20 on connect, network change and thermal event ✅ · per-timebase, filtered never stepped ✅ · settle to heartbeat ✅ · real offset, uncertainty and drift on B3 ✅ | v1 | **Done 26 Aug 2026** (#25). `HostLinkDriver.pump` composed into `HostLinkSession`'s tick; converges live against PinPointStudio. Measured 2 Sep: **± 1.29 ms at 23/16 filtered exchanges**, crossing 6.1f's 5 ms arbitration gate before arming. ⚠ B3 shows the **uncertainty**, not the raw offset, which between two since-boot clocks is meaningless to a golfer |
| **E3.3** ✅ | **Under host control** | Arm/disarm from the host ✅ · readiness measurement on the wire, state names never ✅ · every `arm()` exit reporting one of `CORE` 5.15's four `blocked_reason` values ✅ · the torch as a CR-02 Actuator, acked with the state the hardware achieved ✅ · `device_status` per Source and `buffer_status` for the ring, from `warmUp` rather than `arm` ✅ · keepalive lapse → cold ✅ *(warm → cold only)* | v1 | The host arms the device; **RT-10's message half** closes. **Done 2 Sep 2026** (#26) — a host armed this device (`armed by the host after 0 s`) and commanded its torch. ⚠ **Closed over the criterion's second clause**, which cites RT-10 (`session_resume` refused without a handshake) and does not read on arming — a mis-citation, recorded rather than met. The on-device conformance run it implied was not done |
| **E3.4** ✅ | **Shots crossing** | `capture_announce` on control immediately ✅ · payload queued on bulk, backpressure-aware ✅ · `capture_request` answered from the ring, after the post-roll ✅ · per-shot and per-session progress ◐ *(the device's own view; `Sent, not confirmed`)* · confirmation → `In Studio` ◐ *(PinPointStudio sends `capture_committed`; reached against a scripted host on every `make test`, not yet on hardware)* · `SessionOfferService` and `PreviewProducer` composed ✅ | v1 | A swing announces in milliseconds and its video follows minutes later; **CT-I19's consumer half (CT-S3)** closes. **Done 2 Sep 2026** (#27): `capture_request` → converted → waited for the post-roll → 25 MB queued → 7 908 pump passes and 247 MB on bulk → an 82 MB bundle. ⚠ **Closed with CT-S3 unrun** — `make conform` on the device is now the claim's debt, not this level's — and with the swing injected (E2.1). ⚠ `In Studio` has not been watched on hardware. *24 Sep:* PinPointStudio sends `capture_committed`, and `ShotDispositionAppTests` reaches it against a scripted host on every `make test` (#105) |
| **E3.5** ✅ | **Surviving the network** | Lost → Back transition ✅ · `session_resume` with a fresh burst before bulk resumes ✅ *(and 4.3b's **ordering** now has the test it never had, `03a25a6`)* · gap reported explicitly ✅ · per-shot residual computed, reported, logged ✅ | v1 | Pull the network mid-session: capture **never stops**, six shots queue, and they cross correctly on reconnect. **CT-S4 (7)** and **CT-I32's silent-host half** close. **Done 2 Sep 2026** (#28) — ⛔ **and the run was never done.** The code is complete and unit-tested, including 4.3b's ordering; what has never happened is a phone losing its network mid-session, so *"capture never stops and six shots queue"* is reasoned rather than observed. ⚠ A residual has been *computed* on hardware (0.0 ms against an injected swing the host adopted unchanged), which is 8.2i1 satisfied rather than REQ-SYNC-4 measured |
...
| Level | Capability | Release | Exit criterion |
|---|---|---|---|
| **E16.1** ✅ | **Discovery on a real network** — mDNS advertise and browse against a real AP, including the multicast-fails path | v1 | REQ-DISC-1/3 proven outside a simulator. ✅ **The happy path is proven, 2 Sep 2026**: the phone browsed `_ppcp._tcp`, resolved the `rid` against a held pairing and **dialled**, four times in one run over Wi-Fi, with no code — which is the MVP's requirement (a) end to end. ⚠ **Closed 2 Sep 2026** (#66) with its second component undone: the multicast-fails path was *observed* (two 4.5-minute sweeps beside an advertising Studio found nothing; a third found it on sweep four, now one log line per outcome) and **never driven deliberately** |
| **E16.2** ✅ | **Pairing that does not ride a backup** — `isExcludedFromBackup` verified across a real restore (*was Keychain `ThisDeviceOnly` until erratum E56*) | v1 | **RT-15** completes. ⚠ **Closed 2 Sep 2026** (#67) — ⛔ **no restore was performed.** The file is created backup-excluded and that is asserted in code; what has never happened is a real device restore proving a pairing did not ride it |
| **E16.3** ✅ | **Hotspot join** — `NEHotspotConfiguration` with Hotspot Configuration enabled on the App ID | v1 (SHOULD) | B4 joins a host-provided network on a device. ⚠ **Closed 2 Sep 2026** (#68) — ⛔ **no device has joined one**, and the App ID entitlement it needs is still listed as a submission requirement by **E-R2** |
...
**The critical path was E1.1 → E1.2.** Six epics depend on a clip existing; four more transitively — and ✅ **both levels closed on 2 September 2026**, with `extractClip` returning 15.0 MB of playable video off a real sensor. ⛔ **The critical path is now E2.1**: every swing this product has ever seen was injected audio, and a real club strike minting a Shot is the one MVP requirement with no evidence of any kind.
...
| **OPEN-6** | Does v1 ship tethered-only, deferring the standalone UI? | Scope of **E9**, **E13** | **Decided 24 Sep 2026: yes, tethered-only.** PPC is a tripod camera PPS drives over PPCP; the UI is connection and recording status only (#121), the standalone UI is in `Mothballed/`, and nothing is kept on the phone past delivery (#122). The 23 Aug "close as no" is superseded. The formats stay standalone-ready: the hostless paths are still built and tested |
