...
| **E3.4** ✅ | **Shots crossing** | `capture_announce` on control immediately ✅ · payload queued on bulk, backpressure-aware ✅ · `capture_request` answered from the ring, after the post-roll ✅ · per-shot and per-session progress ◐ *(the device's own view; `Sent, not confirmed`)* · confirmation → `In Studio` ⛔ *(unreachable — no host has sent `capture_committed`)* · `SessionOfferService` and `PreviewProducer` composed ✅ | v1 | A swing announces in milliseconds and its video follows minutes later; **CT-I19's consumer half (CT-S3)** closes. **Done 2 Sep 2026** (#27): `capture_request` → converted → waited for the post-roll → 25 MB queued → 7 908 pump passes and 247 MB on bulk → an 82 MB bundle. ⚠ **Closed with CT-S3 unrun** — `make conform` on the device is now the claim's debt, not this level's — and with the swing injected (E2.1). ⛔ `In Studio` remains unreachable: no host has sent `capture_committed` |
