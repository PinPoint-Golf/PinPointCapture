...
|---|---|---|---|---|
| **E1.1** | **Frames reach the ring** | Wire the live `AVCaptureVideoDataOutput` into `RingBufferRecorder` ○ · encoder config: `ExpectedFrameRate`, `RealTime`, `AllowFrameReordering = false` ○ · fragment index into `Core.FragmentRing`, rollover at 20 ◐ · verify REQ-OPT-1..7 locks hold on hardware ▨ | v1 | Twenty 0.5 s fragments on disk, rolling, at the claimed rate with `alwaysDiscardsLateVideoFrames = false` |
