...
DECISIONS = [
 ...
 ("OPEN-6","Does v1 ship tethered-only, deferring the standalone UI?",
  "Scopes **E9** and **E13**.",
  ["**Decided 24 Sep 2026 (Mark): yes — v1 is tethered-only.** PPC is a camera on a tripod that PPS drives over PPCP, and its UI shows connection and recording status and nothing else (#121).",
   "The standalone UI (onboarding, local Arm, replay, the session library, reconcile) is in `Mothballed/`, out of the build and kept for when offline capture returns.",
   "Nothing is kept on the phone past its delivery (#122), so **E9** has nothing to export and **E13**'s standalone audit has nothing to audit in v1.",
   "The 23 Aug recommendation (\"close as no\") is superseded: the live link now works end to end on hardware, and the product is the tethered camera.",
   "⚠ The formats stay standalone-ready — the hostless code paths are still in the build and still tested; only the app no longer offers them."], None),
